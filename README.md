...
2. `web`: отправляет логи Nginx и Auditd на удаленный сервер.
3. `log`: принимает логи и раскладывает по папкам.

### ✅ Результат
- [x] Стенд развернут (Vagrant + Shell Provisioner).
- [x] Логи Nginx пишутся на удаленный сервер.
- [x] Аудит конфигов настроен.
...
Настройка полностью автоматизирована через Shell Provisioner в `Vagrantfile`.

**Log Server (192.168.56.15):**
* Открыты порты 514 (UDP/TCP).
* Настроен динамический шаблон пути: `/var/log/rsyslog/%HOSTNAME%/%PROGRAMNAME%.log`.
